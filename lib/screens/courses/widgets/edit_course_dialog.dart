...
class EditCourseDialog extends StatefulWidget {
  final Course course;
  final Future<void> Function(Course course, Map<String, dynamic> courseData) editCourse;
...
class _EditCourseDialogState extends State<EditCourseDialog> {
  ...
  void initState() {
    super.initState();
    _fullNameController = TextEditingController(text: widget.course.fullName);
    _shortNameController = TextEditingController(text: widget.course.shortName);
    _roomController = TextEditingController(
      text: widget.course.schedule.isNotEmpty ? widget.course.schedule.first.room : ''
    );

    _dayOfWeek = widget.course.schedule.isNotEmpty ? widget.course.schedule.first.dayOfWeek : null;
    _startDate = DateTime.tryParse(widget.course.startDate);
    _endDate = DateTime.tryParse(widget.course.endDate);

    if (widget.course.schedule.isNotEmpty && widget.course.schedule.first.startTime.isNotEmpty) {
      _startTime = DateTimeUtils.parseTimeOfDay(widget.course.schedule.first.startTime);
    }

    if (widget.course.schedule.isNotEmpty && widget.course.schedule.first.endTime.isNotEmpty) {
      _endTime = DateTimeUtils.parseTimeOfDay(widget.course.schedule.first.endTime);
    }
  }
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate() &&
      ...
      widget.editCourse(widget.course, courseData);
    } else {
